...
static struct k_fifo *cap_fifo1;
static struct k_fifo *cap_fifo2;
static struct k_sem  consumers_done_sem;   /* counts consumer completions */
static int consumers_expected;
...
void setup_cam_capture(struct k_fifo *fifo1, struct k_fifo *fifo2)
{
    cap_fifo1 = fifo1;
    cap_fifo2 = fifo2;
    consumers_expected = 0;
    if (cap_fifo1) consumers_expected++;
    if (cap_fifo2) consumers_expected++;
    k_sem_init(&consumers_done_sem, 0, consumers_expected ? consumers_expected : 2);
...
static void arm_next_spis_transfer(void)
{
    ...
    /* Use Zephyr SPI API properly: device first, then config, then TX/RX */
    spi_transceive(spis_dev.bus, &spis_dev.config, NULL, &rxset);
...
static void line_thread(void *p1, void *p2, void *p3)
{
    ...
    if (atomic_get(&line_idx) >= IMG_HEIGHT) {
        ...
        //printk("ploopt");
        /* Enqueue to all configured consumer FIFOs. Use the same data pointer
         * but separate fifo wrapper items. Each consumer gives the same
         * semaphore when done so we can wait before starting a new frame.
         */
        if (cap_fifo1) {
            frame_item_t *item1 = k_malloc(sizeof(frame_item_t));
            if (item1) {
                item1->data = image;
                item1->len = IMAGE_SIZE;
                item1->consumed_sem = &consumers_done_sem;
                k_fifo_put(cap_fifo1, item1);
            }
        }
        if (cap_fifo2) {
            frame_item_t *item2 = k_malloc(sizeof(frame_item_t));
            if (item2) {
                item2->data = image;
                item2->len = IMAGE_SIZE;
                item2->consumed_sem = &consumers_done_sem;
                k_fifo_put(cap_fifo2, item2);
            }
        }
...
void stream_photos(void)
{
    while(1)
    {
        ...
        /* Wait for all consumers (UART/BLE) to finish using the buffer
         * before allowing the next capture to overwrite it.
         */
        for (int i = 0; i < consumers_expected; i++) {
            k_sem_take(&consumers_done_sem, K_FOREVER);
        }
        /* Frame is already enqueued to FIFOs by the line thread */
    }
